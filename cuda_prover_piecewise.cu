        ...
        const char *preprocessed_path)
{
    ...
    for (size_t i = 0; i < CHUNKS; i++) {
        ...
        if (i == CHUNKS - 1) {
            ...
            gpuErrchk( cudaMemcpyAsync(w1.get(), w_host + (i * B_m_chunked * ELT_BYTES), (B_m_chunked - 1) * ELT_BYTES, cudaMemcpyHostToDevice, sB1) ); 
            gpuErrchk( cudaMemcpyAsync(w2.get(), w_host2 + (i * B_m_chunked * ELT_BYTES), (B_m_chunked - 1) * ELT_BYTES, cudaMemcpyHostToDevice, sB2) ); 
        }
